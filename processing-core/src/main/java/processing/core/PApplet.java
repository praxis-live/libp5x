...
/**
  ...
  */
  ...
  protected PGraphics makeGraphics(int w, int h,
    ...
    try {
      String rndr = renderer;
      PGraphics pg = ServiceLoader.load(GraphicsFactory.class).stream()
              .flatMap(p -> p.get().createGraphics(this, rndr).stream())
 ...
 */
  ...
  */
  ...
  static public void runSketch(final String[] args,
    ...
    disableAWT = Boolean.getBoolean("java.awt.headless");
 ...
 */
  ...
  public void blend(PImage src,
     ...
     */
    public static void main(PApplet sketch) {
        main(sketch, null, null);
    }
     ...
     *
     * @param sketch sketch to run
     * @param options PApplet options
     */
    public static void main(PApplet sketch, List<String> options) {
        main(sketch, options, null);
    }
     ...
     * @param sketch sketch to run
     * @param options PApplet options
     * @param arguments sketch arguments
     */
    public static void main(PApplet sketch, List<String> options, List<String> arguments) {
        List<String> argList = new ArrayList<>();
        if (options != null) {
            argList.addAll(options);
        }
        argList.add(sketch.getClass().getName());
        if (arguments != null) {
            argList.addAll(arguments);
        }
        DefaultMainThreadContext mainThread = new DefaultMainThreadContext();
        setMainThreadContext(mainThread);
        runSketch(argList.toArray(String[]::new), sketch);
        mainThread.run();
    }

    /**
     * A context for running tasks on the main (first) thread of the
     * application. On some OS it is important that various UI operations are
     * handled on this thread. To access the main thread context use
    ...
    /**
     * Override the default main thread context for embedding the library in an
     * application. May only be set once before calling through to
     * {@link PApplet#runSketch(java.lang.String[], processing.core.PApplet)} or
     * {@link PApplet#main(processing.core.PApplet, java.util.List, java.util.List)}.
     *
